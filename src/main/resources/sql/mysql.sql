...
FOR EACH ROW
  BEGIN
    ...
    ELSE
      ...
      SET MESSAGE_TEXT = 'Cannot set category_parent_id = 0, constraint by trigger `set_category_parent_id_on_insert`';
    END IF;
    END IF;
  END$
DELIMITER ;
...
FOR EACH ROW
  BEGIN
    IF NEW.category_parent_id != OLD.category_parent_id
    THEN
      SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Cannot update the category parent id, constraint by trigger `disable_update_category_inherit_structure`';
    END IF;

    IF NEW.category_root_id != OLD.category_root_id
    THEN
      SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Cannot update the category root id, constraint by trigger `disable_update_category_inherit_structure`';
    END IF;

    IF NEW.category_level != OLD.category_level
    THEN
      SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Cannot update the category level, constraint by trigger `disable_update_category_inherit_structure`';
    END IF;
  END$
DELIMITER ;
